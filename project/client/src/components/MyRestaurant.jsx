...
const MyRestaurant = ({ activeTab }) => {
  ...
  const initialState = {
    ...
    from: userRestaurant[0].from || "",
    to: userRestaurant[0].to || "",
    address: userRestaurant[0].address || "",
    description: userRestaurant[0].description || "",
  };
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "medium_price" ? Number(value) : value,
    }));
  };
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("Submitting form data:", formData);
    // await dispatch(updateRestaurant(formData));
  };
  return (
    <>
      <div className="p-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mt-4">
            <div className="flex justify-between">
              ...
              <button
                type="submit"
                className="px-3 py-2 rounded-3xl bg-warning hover:bg-warning/80 active:scale-95"
              >
                Chỉnh sửa
              </button>
            </div>
