...
export const signin = async (req,res) => {
    ...
    try {
        ...
        res.json({success: true, message:"Login successfully!", user: {username: isValid.username, email: isValid.email,image:isValid.image, allergy: isValid.allergy, contact: isValid.contact} , token: token})
...
export const signup = async (req,res) => {
    ...
    try {
        ...
        routelog.push(chalk.green("Create accout", newuser.username ,'successfully!'));
        log(routelog.join(" | "));
        res.json({success: true, message: "Create accout successfully!",user: {username: newuser.username, email: newuser.email,
             image: newuser.image}, token: token});
    } catch (error) {
...
export const profile = async (req,res) => {
    ...
    try {
        ...
        routelog.push(chalk.cyan("Receiver Data Successfully!, email: ", _user.email));
        let result = null;
        if(file){
        ...
        if(_user && _user.image_url && file){
           ...
           const result =  await cloudinary.uploader.destroy(_user.image_url, {invalidate: true});
           console.log(result)
        }
        ...
        const newProfile = {
            ...
            allergy: data.allergy ? data.allergy.split(",").map((a) => a.trim()).filter((a) => a !== "") : user.allergy,
            contact: data.contact.length === 10  ? data.contact : user.contact,
        }
        const newuser = await user.findOneAndUpdate({_id}, newProfile, {new: true});
        routelog.push(chalk.cyan("Replace user profile with new profile..."));
        log(routelog.join(" | "));
        res.json({success: true, message:"Profile changed successfully!", user: newuser})
    } catch (error) {
        routelog.push(chalk.red("Signup error message: ", error.message));
        log(routelog.join(" | "));
...
export const authCheck = async (req,res) => {
    ...
    try {
        ...
        log(routelog.join(" | "));
        res.json({success: true, message: "Auth found successfully!", user :{username: _user.username, email: _user.email
            , image: _user.image, allergy: _user.allergy
         }})
    } catch (error) {
        routelog.push(chalk.red("Signup error message: ", error.message));
        log(routelog.join(" | "));
