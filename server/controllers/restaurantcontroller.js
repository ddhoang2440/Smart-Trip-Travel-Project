...
export const getAllRestaurants = async (req, res) => {
    ...
    try {
        ...
        if (!latitude || !longitude) {
            ...
            { $sort: { rating: -1 } },
            { $lookup: { from: "bookingslots", localField: "_id", foreignField: "restaurant_id", as: "bookingslots" } },
        ])
        }
        else {
            restaurants = await Restaurant.aggregate([
        ...
        routelog.push(chalk.red(`Get All Restaurant error message: ${error.message} `));
        log(routelog.join(" | "));
