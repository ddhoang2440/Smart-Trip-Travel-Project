...
export const searchRestaurant = async (req, res) => {
  const routelog = [];
  routelog.push(chalk.white("Starting Search Restaurant Route:"));
  try {
    const { lat, lng, maxDistance = 2000 } = req.body;
    const restaurants = await Restaurant.aggregate([
      {
        $geoNear: {
    ...
    res.json({ success: false, message: "search restaurant failed!" });
  }
};
