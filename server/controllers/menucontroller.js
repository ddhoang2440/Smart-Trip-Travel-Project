...
const updateRestaurantMediumPrice = async (restaurantId) => {
  try {
    const menus = await Menu.find({ restaurant: restaurantId });
    if (menus.length > 0) {
      const total = menus.reduce((sum, item) => sum + item.price, 0);
      const medium_price = Math.round(total / menus.length);
      await Restaurant.findByIdAndUpdate(restaurantId, { medium_price });
    }
  } catch (error) {
    console.log("Update medium price error:", error.message);
  }
};
