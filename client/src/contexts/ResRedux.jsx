...
export const createRestaurant = createAsyncThunk("/restaurant/create", async (_data, thunkAPI) => {
    try {
        const { data } = await axios.post("restaurant/create", _data, {headers: {Authorization: `Bearer ${localStorage.getItem("token")}`}});
        if(!data.success) {
...
export const getAllRestaurant = createAsyncThunk( "/restaurant/getall", async (_data,thunkAPI) => {
    try {
...
export const getUserRestaurant = createAsyncThunk("/restaurant/user", async (_data, thunkAPI) => {
     try {
      const { data } = await axios.get("/restaurant/user", {headers: {Authorization: `Bearer ${localStorage.getItem("token")}`}});
      if (!data.success) {
        return thunkAPI.rejectWithValue(data.message);
      }
      return data;
...
export const resSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        .addCase(getUserRestaurant.pending, (state) => { state.status = "loading"})
