...
export const profile = createAsyncThunk("/auth/profile" , async ( profile, thunkAPI) => {
    try {
        ...
        if((profile.get("contact").length !== 10 || isNaN(profile.get("contact"))) && profile.get("contact").length !== 0) {
            return thunkAPI.rejectWithValue("Contact number must have 10 word and full of numbers !");
        }
        const {data} = await axios.post("/auth/profile" , profile, {headers: {Authorization: `Bearer ${localStorage.getItem("token")}` }});
        if(!data.success) {
            return thunkAPI.rejectWithValue(data.message)
        }
        return data;
...
export const authCheck = createAsyncThunk("auth/check", async (thunkAPI) => {
    try {
        const { data } = await axios.get("/auth/check", {headers: {Authorization: `Bearer ${localStorage.getItem("token")}`}});
        if(!data.success) {
            return thunkAPI.rejectWithValue(data.message);
        }
        return data;
...
export const deleteAccount = createAsyncThunk("auth/delete", async (thunkAPI) => {
    try {
