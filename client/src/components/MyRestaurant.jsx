...
import { getUserById } from "../contexts/AuthRedux";
import { getBookingById } from "../contexts/BookingRedux";
...
const MyRestaurant = ({ activeTab, setActiveTab, handleEditMenu }) => {
  ...
  }, [comment, dispatch, users]);
  useEffect(() => {
    if (bookingById && bookingById.length > 0) {
      const newUserIds = bookingById
  ...
  }, [bookingById, bookingslotsById, dispatch]);
  ...
  const handleAddTable = (e) => {
    ...
    dispatch(createBookingSlot(newTable)).then((result) => {
  ...
  const handleDeleteTable = async (id) => {
    await dispatch(deleteBookingSlot(id));
    dispatch(getBookingSlot(userRestaurant[0]?._id));
  };
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-500">Doanh thu TB</p>
              <h3 className="text-3xl font-bold mt-2">
                {/* {formatPrice(totalRevenue)}đ */}
              </h3>
            </div>
            <IconReceipt className="text-purple-500" size={32} />
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow">
          <div className="flex items-center justify-between">
      ...
      <div className="bg-white p-6 rounded-xl shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {bookingById.map((data, idx) => {
            const bookingslot = bookingslotsById[data.slot_id];
            return (
              <>
                <div key={idx} className="p-4 border rounded-lg">
                  <p className="font-semibold mb-2">
                    {bookingslot?.time || "Đang tải..."}
                  </p>
                  <div className="h-4 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${
                          data.quantity / bookingslot?.max_slot_2 +
                          bookingslot?.max_slot_4 +
                          bookingslot?.max_slot_8
                        }%`,
                      }}
                    />
                  </div>
                  <div className="flex justify-between mt-2 text-sm">
                    <span>Bàn đặt: {data.quantity}%</span>
                    <span>
                      Bàn trống:{" "}
                      {bookingslot?.max_slot_2 +
                        bookingslot?.max_slot_4 +
                        bookingslot?.max_slot_8}
                      %
                    </span>
    ...
    <div className="space-y-6">
      ...
      {isAddTable && (
        <div className="relative">
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-gray-200/80 p-6 mb-8">
                  ...
                  <div className="bg-linear-to-br from-blue-50 to-indigo-50 rounded-xl p-5 border border-blue-100">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Chọn thời gian</p>
                      </div>
                    </div>
                    <input
                      type="time"
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">Lãng mạn</p>
                          </div>
                        </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-10 pt-8 border-t border-gray-200">
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setIsAddTable(false)}
                    className="px-8 py-3 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 hover:border-gray-400 transition font-semibold flex items-center gap-2"
                  >
                    <IconX size={18} />
                    Hủy bỏ
                  </button>
                  <button
                    type="submit"
                    className="px-8 py-3 bg-linear-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:from-blue-700 hover:to-purple-700 transition font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center gap-2"
                  >
                    <IconCheck size={18} />
                    Xác nhận thêm
                  </button>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left whitespace-nowrap">Ảnh</th>
                <th className="px-6 py-3 text-left whitespace-nowrap">Giá</th>
                <th className="px-6 py-3 text-left whitespace-nowrap">
                  Danh mục
                </th>
                <th className="px-6 py-3 text-left whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-200">
              {usermenu.map((item, idx) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div
                      className="max-w-[150px] truncate text-gray-600 text-sm"
                      title={item.igredient?.join(", ")}
                    >
                      {item.igredient?.length
                        ? item.igredient.join(", ")
                        : "Chưa có nguyên liệu"}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex gap-2">
                      ...
                      <button className="px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 text-sm transition-colors">
                        Xóa
                      </button>
                    </div>
                  </td>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-4">
                    <div className="flex items-center gap-3">
                      <div className="shrink-0">
                        <img
                          className="hidden md:w-12 md:h-12 bg-gray-200 rounded-full object-cover"
                          src={
    ...
    <div className="p-6">
      ...
      <div className="mb-8">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2 border-b">
          {[
            ...
            { id: "tables", label: "Quản lý bàn", icon: IconUsers },
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        {activeSection === "reservations" && (
          <div>cần cái shopcart vô đây ko</div>
        )}
